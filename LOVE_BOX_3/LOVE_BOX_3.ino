...
bool magState_ON = true;
bool lastMagState = HIGH;
...
const uint16_t TEXT_INTERVAL = 1200;
...
// motor pulse time
const uint16_t MOTOR_PULSE  = 150;
...
const uint16_t FADE_STEP_TIME = 30;   // speed of fade
const uint16_t HOLD_TIME = 1200;      // full brightness hold

// text box (centered)
#define TEXT_W 100
#define TEXT_H 18
#define TEXT_Y ((SCREEN_HEIGHT - TEXT_H) / 2)
#define TEXT_X ((SCREEN_WIDTH - TEXT_W) / 2)
...
bool isInTextBox(int x, int y, int s) {
  return (x > TEXT_X - s && x < TEXT_X + TEXT_W + s &&
          y > TEXT_Y - s && y < TEXT_Y + TEXT_H + s);
}

void drawHeartSafe(int x, int y, uint8_t s) {
  if (textActive && isInTextBox(x, y, s)) return;
  drawHeart(x, y, s);
}
...
void loop() {
  ...
  switch (playState) {
    ...
    /* -------- IDLE -------- */
    case IDLE:
      if (magState == HIGH && lastMagState == LOW) {
        dfPlayer.playFolder(1, 1);
        heartbeatEnabled = true;
        playState = SONG1_PLAYING;
      }
      break;

    /* -------- SONG 1 (HEARTS ONLY) -------- */
    case SONG1_PLAYING:
      lovePatterns(now);               // hearts continue
      // ❌ NO text functions here

      if (digitalRead(BUSY) == HIGH) { // song finished
        gapTimer = now;
        playState = GAP_BEFORE_SONG2;
      }
      break;

    /* -------- GAP -------- */
    case GAP_BEFORE_SONG2:
      lovePatterns(now);               // optional: keep hearts alive
      if (now - gapTimer >= 1500) {
        dfPlayer.playFolder(1, 2);     // SONG 2
        textIndex = 0;
        textChangeTimer = now;
        showCenterText(textList[textIndex]);
        playState = SONG2_PLAYING;
      }
      break;
    ...
    case SONG2_PLAYING:
      lovePatterns(now);
      updateCenterText(now);

      if (now - textChangeTimer >= TEXT_INTERVAL && textIndex < TEXT_COUNT - 1) {
        textIndex++;
        showCenterText(textList[textIndex]);
        textChangeTimer = now;
      }

      if (digitalRead(BUSY) == HIGH) { // song 2 finished
...
void lovePatterns(unsigned long now) {
  ...
  for (int i = 0; i < HEART_COUNT; i++) {
    drawHeartSafe(hearts[i].x, hearts[i].y, hearts[i].size);
...
void updateCenterText(unsigned long now) {
  if (!textActive || overlayText == nullptr) return;

  if (now - textTimer < FADE_STEP_TIME) return;
  textTimer = now;
  ...
  if (fadeIn) {
    if (textAlpha < 255) textAlpha += 20;
      ...
      fadeIn = false;
      textTimer = now + HOLD_TIME;
    }
  } else {
    if (textAlpha > 20) textAlpha -= 20;
    else {
      textActive = false;
      overlayText = nullptr;
      return;
  ...
  display.fillRect(TEXT_X - 2, TEXT_Y - 2,
                   TEXT_W + 4, TEXT_H + 4,
                   SSD1306_BLACK);

  display.setTextSize(2);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(TEXT_X + 4, TEXT_Y + 2);
  display.println(overlayText);
}
